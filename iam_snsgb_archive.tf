...
resource "aws_iam_access_key" "dq_snsgb_archive_bucket" {
  user = aws_iam_user.dq_snsgb_archive_bucket.name
}
...
resource "aws_ssm_parameter" "dq_snsgb_archive_bucket_id" {
  name  = "snsgb-archive-bucket-user-id-${local.naming_suffix}"
  type  = "SecureString"
  value = aws_iam_access_key.dq_snsgb_archive_bucket.id
}

resource "aws_ssm_parameter" "dq_snsgb_archive_bucket_key" {
  name  = "snsgb-archive-bucket-user-key-${local.naming_suffix}"
  type  = "SecureString"
  value = aws_iam_access_key.dq_snsgb_archive_bucket.secret
}
