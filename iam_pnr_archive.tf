resource "aws_iam_policy" "dq_pnr_archive_bucket_policy" {
  name = "dq_pnr_archive_bucket_policy"
  ...
  "Statement": [
    {
      ...
      "Effect": "Allow",
      "Resource": [
        "${aws_s3_bucket.dq_pnr_archive_bucket.arn}"
      ]
    },
    {
      "Action": [
...
resource "aws_iam_group_policy_attachment" "dq_pnr_archive_bucket" {
  group      = aws_iam_group.dq_pnr_archive_bucket.name
  policy_arn = aws_iam_policy.dq_pnr_archive_bucket_policy.arn
}

resource "aws_iam_user" "dq_pnr_archive_bucket" {
...
resource "aws_iam_group_membership" "dq_pnr_archive_bucket" {
  name = "dq_pnr_archive_bucket"
