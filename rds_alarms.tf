...
resource "aws_cloudwatch_metric_alarm" "cpu_utilization_too_high" {
  alarm_name          = "$concat{({var.db_instance_id})}-cpu_utilization_too_high"
...
#  ok_actions          = ["${aws_sns_topic.default.arn}"]
  ...
  dimensions {
    DBInstanceIdentifier = "$concat{({var.db_instance_id})}"
  }
}

resource "aws_cloudwatch_metric_alarm" "disk_queue_depth_too_high" {
  alarm_name          = "$concat{({var.db_instance_id})}-disk_queue_depth_too_high"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = "1"
...
resource "aws_cloudwatch_metric_alarm" "freeable_memory_too_low" {
  alarm_name          = "$concat{({var.db_instance_id})}-freeable_memory_too_low"
...
resource "aws_cloudwatch_metric_alarm" "free_storage_space_too_low" {
  alarm_name          = "$concat{({var.db_instance_id})}-free_storage_space_threshold"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = "1"
...
resource "aws_cloudwatch_metric_alarm" "swap_usage_too_high" {
  alarm_name          = "$concat{({var.db_instance_id})}-swap_usage_too_high"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = "1"
